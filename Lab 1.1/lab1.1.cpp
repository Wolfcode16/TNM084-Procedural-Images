...
const float ringDensity = 5.0;

// Assignment: Change the existing pattern in some simple way

// ringDensity: Changes amount of rings
// Introduce noise with sin() + fract()
// Change color with ptex
...
void maketexture()
{
	...
	{
	    // Shift around the cirlces center
		fx = (float)(x-kTextureSize/10.)/kTextureSize*2.;
		fy = (float)(y-kTextureSize/2.)/kTextureSize*2.;

        // Introduce some noise with fract
		fxo = fract(sqrt(fx*fx+fy*fy)*20);
		fyo = fract(sqrt(tan(fx*fx+fy*fy))*2);

        fxo = fract(cos(fxo * ringDensity)*4);
		fyo = fract(sin(fyo * ringDensity)*10);
		...
		ptex[x][y][0] = 255;                // R
		ptex[x][y][1] = fyo * 128 + 128;    // G
		ptex[x][y][2] = fxo * 128;          // B
	}
}
