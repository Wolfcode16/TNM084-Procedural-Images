...
{
    float total = 0.0;      // Accumulated noise value
    ...
    for (int i = 0; i < octaves; i++)
    {
        ...
        total += snoise2(x * frequency, z * frequency) * amplitude;

        // Keep track of max possible value for normalization
        maxValue += amplitude;

        // Increase frequency (makes details smaller)
        frequency *= lacunarity;

        // Decrease amplitude (makes contribution weaker)
...
// Helper function to get a vertex position around edges
    vec3 getVertex(int x, int z)
    {
        // Clamp to valid range to handle edges
        if (x < 0) x = 0;                           // Less than 0
        if (x >= kTerrainSize) x = kTerrainSize - 1;// Keep within terrainsize
        if (z < 0) z = 0;                           // Less than 0
        if (z >= kTerrainSize) z = kTerrainSize - 1;// Keep within terrainsize

        return vertices[z * kTerrainSize + x];
    }
...
void MakeTerrain()
{
	...
	// FBM Parameters
	int octaves = 6;             // More layers = more details
	float persistence = 0.5;    // How much each octave contributes
	float lacunarity = 2.0;     // Frequency multiplier between octaves
	float noiseScale = 0.025;    // Overall scale of terrain features (small = big features)
	float heighMult = 10.0;     // How tall the terrain gets
...
//		float y = MAX(0, 3-h) * bumpHeight;
        ...
        // noise Value using FBM
        float noiseValue = fbm(x * noiseScale, z * noiseScale, octaves, persistence, lacunarity);

        float y = noiseValue * heighMult;
	...
	for (int z = 0; z < kTerrainSize; z++)
	{
		...
		int ix = z * kTerrainSize + x;
		vec3 center = vertices[ix];

		// Get neighboring vertices (with edge handling via getVertex)
		vec3 left   = getVertex(x - 1, z);      // West neighbor
        vec3 right  = getVertex(x + 1, z);      // East neighbor
        vec3 down   = getVertex(x, z - 1);      // South neighbor
        vec3 up     = getVertex(x, z + 1);      // North neighbor

        // Calculate edge vectors from center to neighbors
        vec3 vecLeft  = VectorSub(left, center);
        vec3 vecRight = VectorSub(right, center);
        vec3 vecDown  = VectorSub(down, center);
        vec3 vecUp    = VectorSub(up, center);

        // Normal from top-left triangle
        vec3 n1 = CrossProduct(vecUp, vecLeft);

        // Normal from top-right triangle
        vec3 n2 = CrossProduct(vecRight, vecUp);

        // Normal from bottom-right triangle
        vec3 n3 = CrossProduct(vecDown, vecRight);

        // Normal from bottom-left triangle
        vec3 n4 = CrossProduct(vecLeft, vecDown);

        // Add all four normals
        vec3 normal = VectorAdd(n1, VectorAdd(n2, VectorAdd(n3, n4)));
